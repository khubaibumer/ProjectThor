...
PRIVATE void __accept(void *ptr) {
...
PRIVATE void print_nodes(data_node_t *node, char *arg) {
...
PRIVATE void __list(void *ptr) {
    ...
    CAST(ptr)->rpc.return_value.ret.value[CAST(ptr)->rpc.return_value.ret.len
                                          - 1] = ']';
    CAST(ptr)->rpc.return_value.ret.value[CAST(ptr)->rpc.return_value.ret.len] = 0x00;
}

PRIVATE void __kick(void *ptr, int _fd) {
    if (CAST(ptr)->use_ssl) {
...
PRIVATE void __close_client(void *ptr) {
...
PRIVATE void close_all_clients(data_node_t *node, void *ptr) {
